...
function GetCurContent($body)
{
    ...
    {
//阿里云OSS
if(preg_match("#".$cfg_oss_domain."#i", $value))
{
	continue;
}
        ...
        if($rs)
        {
            ...
            $body = str_replace($value, $cfg_oss_domain.$fileurl, $body);
            @WaterImg($rndFileName, 'down');
//阿里云OSS
if($cfg_oss == 'Y')
{
	$ossClient = new OssClient($cfg_oss_key_id, $cfg_oss_key_secret, $cfg_oss_endpoint);
	$ossClient->uploadFile($cfg_oss_bucket, preg_replace('/\//','',$fileurl,1), str_replace('/', DIRECTORY_SEPARATOR,$rndFileName));
}
        }
    }
    $htd->Close();
...
function GetDDImage($litpic, $picname, $isremote)
{
	...
	global $cfg_oss, $cfg_oss_key_id, $cfg_oss_key_secret, $cfg_oss_bucket, $cfg_oss_domain;//阿里云OSS
    $ntime = time();
    ...
    else
    {
        ...
        if($isremote==1 && preg_match("#^http:\/\/#i", $picname) && strpos($picname,$cfg_oss_domain) === false)//阿里云OSS
        {
            $litpic = $picname;
...
//阿里云OSS
if($cfg_oss == 'Y' && $litpic != '' && strpos($litpic,$cfg_oss_domain) === false)
{
	$ossClient = new OssClient($cfg_oss_key_id, $cfg_oss_key_secret, $cfg_oss_endpoint);
	$ossClient->uploadFile($cfg_oss_bucket, preg_replace('/\//','',$litpic,1), str_replace('/', DIRECTORY_SEPARATOR,$cfg_basedir.$litpic));
}
    return $cfg_oss_domain.$litpic;
}
...
{
	$ossClient = new OssClient($cfg_oss_key_id, $cfg_oss_key_secret, $cfg_oss_endpoint);
	$ossClient->uploadFile($cfg_oss_bucket, preg_replace('/\//','',$ddpicok,1), str_replace('/', DIRECTORY_SEPARATOR,$toFile));
}
    $inquery = "INSERT INTO `#@__uploads`(arcid,title,url,mediatype,width,height,playtime,filesize,uptime,mid)
                    VALUES ('0','$filename','$cfg_oss_domain"."$ddpicok','1','0','0','0','$fsize','".time()."','".$cuserLogin->getUserID()."'); ";
    $dsql->ExecuteNoneQuery($inquery);
    $fid = $dsql->GetLastID();
