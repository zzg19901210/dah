    ...
    if(is_array($_SESSION['bigfile_info']))
    {
        foreach($_SESSION['bigfile_info'] as $k=>$v)
        {
            ...
            if(!$hasone && $ddisfirst==1 && $litpic=='')
            {
				...
				$litpic = ($cfg_oss == 'Y') ? $cfg_oss_domain.$litpic : $litpic;//阿里云OSS
                $hasone = TRUE;
            }
            $imginfo =  !empty(${'picinfook'.$k}) ? ${'picinfook'.$k} : '';
            $imgurls .= "{dede:img ddimg='".$cfg_oss_domain.$v."' text='$imginfo' width='".$imginfos[0]."' height='".$imginfos[1]."'} ".$cfg_oss_domain.$v." {/dede:img}\r\n";//阿里云OSS
        }
    }
