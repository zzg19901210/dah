...
if($cfg_oss == 'Y')
{
	$imgfile = str_replace($cfg_oss_domain, '', $imgfile);
}
            $tmpFile = ${'imgfile'.$i};
...
if($cfg_oss == 'Y')
{
	$imgfilestr = str_replace($cfg_basedir, '', $imgfile);
	$object = preg_replace('/\//','',$imgfilestr,1);
	$ossClient->deleteObject($cfg_oss_bucket, $object);
	$ossClient->uploadFile($cfg_oss_bucket, $object, str_replace('/', DIRECTORY_SEPARATOR,$imgfile));
	$litpicname = $iurl = $cfg_oss_domain.$imgfilestr;
}
    ...
    if(is_array($_SESSION['bigfile_info']))
    {
        foreach($_SESSION['bigfile_info'] as $k=>$v)
        {
            ...
            $imgurls .= "{dede:img ddimg='".$cfg_oss_domain.$litpicname."' text='$imginfo' width='".$imginfos[0]."' height='".$imginfos[1]."'} ".$cfg_oss_domain.$v." {/dede:img}\r\n";//阿里云OSS
        }
    }
