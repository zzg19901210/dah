...
if($cfg_oss == 'Y')
{
	...
	$object = preg_replace('/\//','',$ddpicok,1);
	$ossClient->deleteObject($cfg_oss_bucket, $object);
	$ossClient->uploadFile($cfg_oss_bucket, $object, str_replace('/', DIRECTORY_SEPARATOR,$ddpicokurl));
}
    ...
    if( empty($isupload) )
    {
        ...
        VALUES ('$ddpicok','$cfg_oss_domain"."$ddpicok','1','0','0','0','".filesize($ddpicokurl)."','".time()."','".$cuserLogin->getUserID()."'); ";
...
ReturnImg("<?php echo $cfg_oss_domain.$ddpicok; ?>");
</SCRIPT>
<?php
}
?>