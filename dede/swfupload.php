...
if($cfg_oss == 'Y')
{
	$ossClient->uploadFile($cfg_oss_bucket, substr($filedir,1).'/'.$filename.$sname, str_replace('/', DIRECTORY_SEPARATOR,$cfg_basedir.$fileurl));
}

    $inquery = "INSERT INTO `#@__uploads`(title,url,mediatype,width,height,playtime,filesize,uptime,mid)
        VALUES ('$title','".$cfg_oss_domain.$fileurl."','0','0','0','".filesize($cfg_basedir.$fileurl)."','".time()."','".$cuserLogin->getUserID()."'); ";
	if(!empty($arcid))
	{
		$inquery = "INSERT INTO `#@__uploads`(arcid,title,url,mediatype,width,height,playtime,filesize,uptime,mid)
        VALUES ('$arcid','$title','".$cfg_oss_domain.$fileurl."','$ftype','0','0','0','".filesize($cfg_basedir.$fileurl)."','".time()."','".$cuserLogin->getUserID()."'); ";
	}
...
else if($dopost=='del')
{
    ...
    $dsql->ExecuteNoneQuery("DELETE FROM `#@__uploads` WHERE url LIKE '".$cfg_oss_domain.$_SESSION['bigfile_info'][$id]."'; ");
    @unlink($cfg_basedir.$_SESSION['bigfile_info'][$id]);
//阿里云OSS
if($cfg_oss == 'Y')
{
	$ossClient->deleteObject($cfg_oss_bucket, preg_replace('/\//','',$_SESSION['bigfile_info'][$id],1));
}
...
else if($dopost=='addtoedit')
{
	...
	echo ($cfg_oss == 'Y') ? $cfg_oss_domain.$_SESSION['bigfile_info'][$id] : $_SESSION['bigfile_info'][$id];//阿里云OSS
	exit();
}
/************************
//获取本地图片的缩略预览图
...
if($cfg_oss == 'Y')
{
	$picfile = str_replace($cfg_oss_domain, '', $picfile);
	$object = preg_replace('/\//','',$picfile,1);
	$ossClient->deleteObject($cfg_oss_bucket, $object);
}
    ...
    $dsql->ExecuteNoneQuery("DELETE FROM `#@__uploads` WHERE url LIKE '".$cfg_oss_domain.$picfile."'; ");
    echo "<b>已删除！</b>";
    exit();
}