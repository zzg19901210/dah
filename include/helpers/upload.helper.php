...
if($cfg_oss == 'Y')
{
	...
	$ossClient->uploadFile($cfg_oss_bucket, preg_replace('/\//','',$fileurl,1), str_replace('/', DIRECTORY_SEPARATOR,$cfg_basedir.$fileurl));
}

        //保存信息到数据库
        $title = $filename.'.'.$file_sname;
        $inquery = "INSERT INTO `#@__uploads`(title,url,mediatype,width,height,playtime,filesize,uptime,mid)
            VALUES ('$title','$cfg_oss_domain"."$fileurl','$filetype','0','0','0','".filesize($cfg_basedir.$fileurl)."','".time()."','".$cuserLogin->getUserID()."'); ";
