...
function GetFieldValue($dvalue, $dtype, $aid=0, $job='add', $addvar='', $admintype='admin', $fieldname='')
{
    ...
    else if($dtype=='img' || $dtype=='imgfile')
    {
        ...
        if(preg_match("/^http:\/\//i", $iurl) && $GLOBALS['cfg_isUrlOpen'] && strpos($iurl,$cfg_oss_domain) === false)//阿里云OSS
        {
            //远程图片
            $reimgs = '';
...
if($cfg_oss == 'Y')
{
	$ossClient->uploadFile($cfg_oss_bucket, preg_replace('/\//','',$reimgs[0],1), str_replace('/', DIRECTORY_SEPARATOR,$cfg_basedir.$reimgs[0]));
	$reimgs[0] = $cfg_oss_domain.$reimgs[0];
}
                    if($dtype=='imgfile')
                    {
                        $imgurl = $reimgs[0];//阿里云OSS
                    }
                    else
                    {
        ...
        {
            //站内图片
			$imgfile = ($cfg_oss == 'Y') ? $iurl : $cfg_basedir.$iurl;//阿里云OSS
            if(is_file($imgfile))
            {
                $info = '';
